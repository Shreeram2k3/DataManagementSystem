<?php

namespace App\Http\Controllers\FacultyActivityController;
use App\Models\FacultyActivityModels\FA_XI;
use App\Http\Controllers\Controller;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Storage;

class FA_XIController extends Controller
{
    public function store(Request $request)
    {
        $type='FA_XI';
        try{
            $validate=$request->validate([
                'name_of_staff' => 'required|date',
                'name_of_the_programme' => 'required|string|max:255',
                'BIT_Outside' => 'required|string|max:255',
                'Duration' => 'required|string|max:255',
                'Outcome' => 'required|string|max:255',
                'dept' => 'required|string|max:255',
                'document_link' => 'nullable|url',
                'document' => 'required|file|mimes:pdf,doc,docx|max:5120'
            ]);

            $validated['user_id'] = auth()->id();
              if ($request->hasFile('document')) {
                $file = $request->file('document');
                $filename = time() . '_' . $file->getClientOriginalName(); //adding timestamp to avoid collisions
                $validated['document'] = $file->storeAs('FA_Documents/FA_XI', $filename, 'public');
            }
            try{

            // left side column name in table, right side name attribute in form 
            FA_XI::create([
                'Name_of_Staff' => $validated['name_of_staff'],
                'Name_of_the_Programme' => $validated['name_of_the_programme'],
                'BIT/Outside' => $validated['BIT_Outside'],
                'Duration' => $validated['Duration'],
                'Outcome' => $validated['Outcome'],
                'Dept' => $validate['dept'],
                'Document_Link' => $validated['document_link'],
                'Document'=>$validated['document'],
                
                
                'user_id' => $validated['user_id']
            ]);
           return back()->with('success', 'Faculty  activity added successfully.');

           
           }
              catch (\Exception $e) {
                   dd($e->getMessage());
                }

        }
        catch (\Exception $e) {
            dd($e->getMessage());
            return redirect()->back()->withErrors(['error' => 'Failed to create student activity: ' . $e->getMessage()]);
        }
    }

    //---------------- ----------------------------------

    public function update(Request $request, $id)
    {
            $record = FA_XI::findOrFail($id);

            // Validate input
            $request->validate([
                'name_of_staff' => 'required|date',
                'name_of_the_programme' => 'required|string|max:255',
                'BIT_Outside' => 'required|string|max:255',
                'Duration' => 'required|string|max:255',
                'Outcome' => 'required|string|max:255',
                'dept' => 'required|string|max:255',
                'document_link' => 'nullable|url',
                'document' => 'nullable|file|mimes:pdf,doc,docx|max:5120'
            ]);

            // Update fields
            $record->Name_of_Staff = $request->input('name_of_staff');
            $record->Name_of_the_Programme = $request->input('name_of_the_programme');
            $record['BIT/Outside'] = $request->input('BIT_Outside');
            $record->Duration = $request->input('Duration');
            $record->Outcome = $request->input('Outcome');
            $record->Dept = $request->input('dept');
            $record->document_link = $request->input('document_link');
            

            // If a new document is uploaded
    if ($request->hasFile('document')) {
        // Delete old file if exists
        if ($record->Document && Storage::disk('public')->exists($record->Document)) {
            Storage::disk('public')->delete($record->Document);
        }

        // Save new file
        $file = $request->file('document');
        $filename = time() . '_' . $file->getClientOriginalName();
        $record->Document = $file->storeAs('FA_Documents/FA_XI', $filename, 'public');
    }
    // else → keep old file

            $record->save();

            return redirect()->route('FA.view', ['type' => 'FA_XI'])->with('success', 'Faculty activity updated successfull');
    }
}
