...
class FA_VIController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            $validated = $request->validate([
                ...
                'organiser_details'=>'required|string|max:255',
                'from_date' => 'required|date',
                'to_date' => 'required|date',
                'purpose_of_attending'=>'required|string|max:255',
    ...
    if ($request->hasFile('document')) {
        ...
        $record->Document = $file->storeAs('FA_Documents/FA_VI', $filename, 'public');
    }
    // else → keep old file

            $record->save();

            return redirect()->route('FA.view', ['type' => 'FA_VI'])->with('success', 'Faculty activity updated successfull');
    }
