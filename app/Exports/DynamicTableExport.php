...
class DynamicTableExport implements FromCollection, WithTitle, WithHeadings, WithEvents
{
    ...
    public function __construct(string $modelClass, string $sheetName, string $fromDate, string $toDate, ?string $userDepartment = null)
    {
    ...
    public function collection()
    {
        ...
        // Admin filter using user relationship
        if ($this->userDepartment) {
            $query->whereHas('user', function ($q) {
                $q->whereRaw('LOWER(department) = ?', [strtolower($this->userDepartment)]);
            });
        }
        ...
        $data->transform(function ($item) use (&$counter, $removeColumns) {
            ...
            foreach ($removeColumns as $col) {
                if (isset($item->$col)) unset($item->$col);
            }
            ...
            if (isset($item->document) && !empty($item->document)) {
                $folder = strtoupper($this->sheetName);
        ...
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->getStyle('A1')->applyFromArray([
                    'font' => ['bold' => true, 'size' => 14, 'color' => ['rgb' => '1A1A1A'], 'name' => 'Calibri'],
                    'alignment' => [
                ...
                // 2. Header row with gradient
                $sheet->getStyle("A2:{$highestColumn}2")->applyFromArray([
                    'font' => ['bold' => true, 'size' => 12, 'color' => ['rgb' => 'FFFFFF'], 'name' => 'Segoe UI'],
                    'alignment' => [
                        'horizontal' => \PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER,
                        'vertical' => \PhpOffice\PhpSpreadsheet\Style\Alignment::VERTICAL_CENTER,
                        'wrapText' => true,
                ...
                $sheet->getStyle("A2:{$highestColumn}{$lastRow}")->applyFromArray([
                    'borders' => [
                        'inside' => ['borderStyle' => \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_HAIR, 'color' => ['rgb' => 'DDDDDD']],
                        'outline' => ['borderStyle' => \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN, 'color' => ['rgb' => 'AAAAAA']],
                    ],
                ]);
                ...
                $sheet->getStyle("A{$footerRow}")->applyFromArray([
                    'font' => ['italic' => true, 'size' => 10, 'color' => ['rgb' => '666666'], 'name' => 'Calibri Light'],
