...
<main class="w-full lg:w-1/2 bg-white px-6 py-8 sm:px-10 lg:px-20 flex items-center justify-center">
    <form id="facultyForm" method="POST" 
        action="{{ isset($record) ? route('FAII_update', ['type' => $type, 'id' => $record->S_NO]) : route('FAII_Store') }}" 
        ...
        <label class="block">
            <span class="text-sm text-gray-600">Details of publication</span>
            <input type="text" name="details_of_publication" required
                value="{{ $record->Details_of_publication ?? old('Details_of_publication') }}"
                class="w-full border-b border-pink-400 focus:outline-none focus:border-pink-600 py-2">
        </label>
