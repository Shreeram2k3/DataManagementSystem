...
<main class="w-full lg:w-1/2 bg-white px-6 py-8 sm:px-10 lg:px-20 flex items-center justify-center">
    <form id="facultyForm" method="POST" 
        ...
        <label class="block">
            <span class="text-sm text-gray-600">Date_of_Recognition</span>
            <input type="text" name="date_of_recognition" required
                value="{{ $record->Date_of_Recognition ?? old('Date_of_Recognition') }}"
                class="w-full border-b border-pink-400 focus:outline-none focus:border-pink-600 py-2">
        </label>

        <label class="block">
            <span class="text-sm text-gray-600">Reference_No</span>
