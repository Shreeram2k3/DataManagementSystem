...
<main class="w-full lg:w-1/2 bg-white px-6 py-8 sm:px-10 lg:px-20 flex items-center justify-center">
    <form id="facultyForm" method="POST" 
        action="{{ isset($record) ? route('FAXVII_update', ['type' => $type, 'id' => $record->S_NO]) : route('FAXVII_Store') }}" 
        ...
        <label class="block">
            <span class="text-sm text-gray-600">Period/Date</span>
            <input type="text" name="period_date" required
                value="{{ $record['Period/Date'] ?? old('Period/Date') }}"
                class="w-full border-b border-pink-400 focus:outline-none focus:border-pink-600 py-2">
        </label>
