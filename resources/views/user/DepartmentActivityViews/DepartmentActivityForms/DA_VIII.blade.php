...
<main class="w-full lg:w-1/2 bg-white px-6 py-8 sm:px-10 lg:px-20 flex items-center justify-center">
    <form id="facultyForm" method="POST" 
        action="{{ isset($record) ? route('DAVIII_update', ['type' => $type, 'id' => $record->S_NO]) : route('DAVIII_Store') }}" 
        ...
        <label class="block">
            <span class="text-sm text-gray-600">Name of the Board</span>
            <input type="text" name="name_of_the_board" required
                value="{{ $record->Name_of_the_Board ?? old('Name_of_the_Board') }}"
                class="w-full border-b border-pink-400 focus:outline-none focus:border-pink-600 py-2">
        </label>

        <label class="block">
            <span class="text-sm text-gray-600">BOS Members attended</span>
            <input type="text" name="bos_members_attended" required
                value="{{ $record->BOS_Members_attended ?? old('BOS_Members_attended') }}"
                class="w-full border-b border-pink-400 focus:outline-none focus:border-pink-600 py-2">
        </label>
