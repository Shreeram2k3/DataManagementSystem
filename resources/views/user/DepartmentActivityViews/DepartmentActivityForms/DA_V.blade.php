...
<main class="w-full lg:w-1/2 bg-white px-6 py-8 sm:px-10 lg:px-20 flex items-center justify-center">
    <form id="facultyForm" method="POST" 
        action="{{ isset($record) ? route('DAV_update', ['type' => $type, 'id' => $record->S_NO]) : route('DAV_Store') }}" 
        ...
        <label class="block">
            ...
            <input type="text" name="volume" required
                value="{{ $record->Volume ?? old('Volume') }}"
                class="w-full border-b border-pink-400 focus:outline-none focus:border-pink-600 py-2">
        </label>

        <label class="block">
            <span class="text-sm text-gray-600">Release</span>
            <input type="text" name="release" required
                value="{{ $record->Release ?? old('Release') }}"
                class="w-full border-b border-pink-400 focus:outline-none focus:border-pink-600 py-2">
        </label>
