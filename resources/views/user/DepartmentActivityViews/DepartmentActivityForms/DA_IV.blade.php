...
<main class="w-full lg:w-1/2 bg-white px-6 py-8 sm:px-10 lg:px-20 flex items-center justify-center">
    <form id="facultyForm" method="POST" 
        action="{{ isset($record) ? route('DAIV_update', ['type' => $type, 'id' => $record->S_NO]) : route('DAIV_Store') }}" 
        ...
        <label class="block">
            <span class="text-sm text-gray-600">Name of VIPs/Inspection Commission</span>
            <input type="text" name="name_of_VIPs/inspection_commission" required
                value="{{ $record['Name_of_VIPs/Inspection_Commission'] ?? old('Name_of_VIPs/Inspection_Commission') }}"
